...
// Native AI Gateway: if the three gateway secrets are set but CF_AI_GATEWAY_MODEL is missing,
// default to Anthropic Sonnet (README default). OpenAI-through-gateway users should set
// CF_AI_GATEWAY_MODEL explicitly, e.g. openai/gpt-4o
(function () {
    ...
    if (accountId && gatewayId && apiKey) {
        process.env.CF_AI_GATEWAY_MODEL = 'anthropic/claude-sonnet-4-5';
        console.log(
            'CF_AI_GATEWAY_MODEL unset; defaulting to anthropic/claude-sonnet-4-5 (set wrangler secret to override)',
        );
    }
})();
...
if (!primaryModel) {
    console.warn(
        'No agents.defaults.model.primary after patch — Control UI may show "model not chosen". ' +
            'Set wrangler secret CF_AI_GATEWAY_MODEL (e.g. anthropic/claude-sonnet-4-5 or openai/gpt-4o) ' +
            'or choose a model in the UI.',
    );
}
